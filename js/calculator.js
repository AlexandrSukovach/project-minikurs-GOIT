...
function renderSum(sum) {
  const coastElement = document.querySelector('.calculator-form-total-coast');

  coastElement.textContent = 'Calculating...'

setTimeout(function() {
  coastElement.textContent = sum + '$';
}, 1000);
...
function convertCartOptionToPrice (options) {
 ...
 function convertReceptionOptionToPrice (options) {
  if (options === 'yes') {
    return 500;
  } 
  return 0;
